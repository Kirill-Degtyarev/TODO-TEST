...
export default class CardHistoryAction {
    ...
    static async getHistory(setHistory, cardId) {
        onSnapshot(
            query(collection(db, 'cardHistory'), where('card', '==', cardId)),
            async (docs) => {
                let history = [];
                docs.forEach((doc) => {
                    ...
                    history.push(
                        new CardHistory(
                            ...
                            historyData.type,
                            historyData.date,
                            historyData.creatorname,
                            historyData.creatoremail,
                            historyData.creatoravatar,
                            historyData.delay,
                            historyData.comment,
                            historyData.card,
                        ),
                ...
                for (let index = 0; index < history.length; index++) {
                    history[index].files = await FileAction.getHistoryFiles(
                        history[index].cardId,
                        history[index].id,
                    );
                }
                history.sort((a, b) => {
