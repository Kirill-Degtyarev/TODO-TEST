...
export default class CardsAction {
    ...
    static async getBoardCard(boardId) {
        const cards = [];
        await getDocs(query(collection(db, 'cards'), where('board', '==', boardId))).then(
            (cardDocs) => {
    ...
    static async updateCardDate(newCardDate, card, workSpaceId) {
        ...
        switch (userDelay) {
            ...
            case 'удалил(-а)':
                ...
                break;
            case 'изменил(-а)':
                historyDelay += `${userDelay} срок сдачи с ${formatter.format(
                    card.date,
                )} на ${formatter.format(newCardDate)}`;
                break;
            default:
                break;
        }
    ...
    static async removeUserFromCard(email, card, name, workSpaceId) {
        ...
        let historyDelay = `${auth.currentUser.displayName || auth.currentUser.email} убрал(-а) ${
            name || email
        } из карточки`;
        await CardHistoryAction.addHistroy('move', historyDelay, [], card, workSpaceId);
    }
    static async addUserInCard(email, card, name, workSpaceId) {
        ...
        let historyDelay = `${auth.currentUser.displayName || auth.currentUser.email} добавил(-а) ${
            name || email
        } в карточку`;
        await CardHistoryAction.addHistroy('move', historyDelay, [], card, workSpaceId);
    }
