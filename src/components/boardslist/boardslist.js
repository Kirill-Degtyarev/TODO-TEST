...
export default function BoardList({ boards, setBoards, user, workSpace }) {
    ...
    function dropOnBoard(e, board) {
        ...
        if (currentCardForBoard !== null) {
            ...
            const fromBoard = boards.find((b) => b.id === currentCardForBoard.boardId);
            ...
            if (fromBoard !== board) {
                CardHistoryAction.addHistroy(
                    'move',
                    `${user.displayName || user.email} переместил(-а) карточку из ${
                        fromBoard.name
                    } в ${board.name}`,
                    [],
                    currentCardForBoard,
    ...
    function addBoard() {
        const sortIndex = boards.length > 0 ? boards[boards.length - 1].sortIndex + 1 : 1;
    ...
    async function addCard(e, boardId) {
        if (e.target.value !== '') {
            ...
            const newCard = new Card('', e.target.value, null, sortIndex, boardId, null, null, 0);
                      ...
                      return (
                          ...
                          >
                              <div className="board-name-container">
                                  ...
                                  {cards.filter((c) => c.boardId === board.id).length === 0 ? (
                                      <img
                                          onClick={async () => {
                                              await BoardsAction.deleteBoard(board.id);
                                          }}
                                          src={basketImage}
                              ...
                              <div className="board-cards-container">
                                  {cards ? (
                                      <CardList
                                          ...
                                          setCurrentCardForBoard={setCurrentCardForBoard}
